...
2. **Resolve configuration** by running:

    ```bash
    python3 {project-root}/_bmad/scripts/resolve_config.py --project-root {project-root} --key core --key agents
    ```

    The resolver merges four layers in order: `_bmad/config.toml` (installer base, team-scoped), `_bmad/config.user.toml` (installer base, user-scoped), `_bmad/custom/config.toml` (team overrides), and `_bmad/custom/config.user.toml` (personal overrides).

    - Use `core.user_name` for greeting
    - Use `core.communication_language` for all communications
    - Build an internal roster of available agents from the `agents` array. Each entry is keyed by the agent's `code` and carries `name`, `title`, `icon`, `description`, `module`, and `team`.
