...
- **Catalog**: `{project-root}/_bmad/module-help.csv` — consolidated manifest of all installed module skills
- **Config**: `config.toml` and `config.user.toml` files in `{project-root}/_bmad/` — resolve `output-location` variables, provide `communication_language` and `project_knowledge`
