#!/usr/bin/env python3
"""
Resolve Maia's central config using four-layer TOML merge.

Reads from four layers (highest priority last):
  1. {project-root}/_maia/config.toml              (installer-owned team)
  2. {project-root}/_maia/config.user.toml         (installer-owned user)
  3. {project-root}/_maia/custom/config.toml       (human-authored team, committed)
  4. {project-root}/_maia/custom/config.user.toml  (human-authored user, gitignored)
...
def main():
    parser = argparse.ArgumentParser(
        description="Resolve Maia central config using four-layer TOML merge.",
    )
    parser.add_argument(
        "--project-root", "-p", required=True,
        help="Absolute path to the project root (contains _maia/)",
    )
    parser.add_argument(
    ...
    maia_dir = project_root / "_maia"

    base_team = load_toml(maia_dir / "config.toml", required=True)
    base_user = load_toml(maia_dir / "config.user.toml")
    custom_team = load_toml(maia_dir / "custom" / "config.toml")
    custom_user = load_toml(maia_dir / "custom" / "config.user.toml")
