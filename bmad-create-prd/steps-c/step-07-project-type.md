...
### 1. Identify Technical Stack Context

**Assess Conversation for Provided Stack:**
- If the user has already specified their stack (e.g., FastAPI, Next.js, PostgreSQL):
  - **STOP** using generic CSV lookups for questions
  - **ADOPT** the specific requirements of that stack immediately
  - Focus discovery on: "How do these specific technologies interact in your internal environment?"
  - Skip "Key Questions" from the CSV that are already answered by the stack choice (e.g., don't ask about "Authentication method" if you can infer or ask about "FastAPI security patterns" instead).

### 2. Streamlined Configuration (If Stack Provided)

If the stack is known, bypass the CSV-driven discovery and use the following "Internal Power Stack" sections:
- **Backend (e.g. FastAPI/Node):** Focus on internal endpoints, service-to-service auth, and internal API documentation.
- **Frontend (e.g. Next.js):** Focus on internal dashboard layout, SSO integration, and state management for internal workflows.
- **Database (e.g. PostgreSQL):** Focus on internal schema design, data migration from legacy internal systems, and reporting needs.

**If stack is NOT provided, proceed with standard CSV lookup:**
- Lookup data in `../data/project-types.csv`
- Follow the `key_questions` and `required_sections` as defined.
