...
ticket_key: '' # set at runtime when discovered from sprint status
---
...
1. **Find the review target.** The conversation context before this skill was triggered IS your starting point — not a blank slate. Check in this order — stop as soon as the review target is identified:
   ...
   **Tier 3 — Sprint tracking.**
   Look for a sprint status file (`*sprint-status*`) in `{implementation_artifacts}` or `{planning_artifacts}`. If found, scan for tickets with status `review`:
   - **Exactly one `review` ticket:** Set `{ticket_key}` to the ticket's key (e.g., `1-2-1-backend-setup`). Suggest it: "I found ticket <ticket-id> in `review` status. Would you like to review its changes? [Y] Yes / [N] No, let me choose". If confirmed, use the ticket context to determine the diff source (branch name derived from ticket slug, or uncommitted changes). If declined, clear `{ticket_key}` and fall through.
   - **Multiple `review` tickets:** Present them as numbered options alongside a manual choice option. Wait for user selection. If a ticket is selected, set `{ticket_key}` and use its context to determine the diff source. If manual choice is selected, clear `{ticket_key}` and fall through.
   - **None:** Fall through.
